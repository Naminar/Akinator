...
cStack::cStack(int capacity, void* stack_place)
{
    ...
    this->data[0] = (DATA) CANARY;
    this->data[capacity + 1] = (DATA) CANARY;
...
int cStack::stack_resize()
{
    ...
    this->data[this->capacity + 1] = (DATA) CANARY;
...
{
    if(this->first_canary != (DATA) CANARY || this->second_canary != (DATA) CANARY)
        return -1;

    if(this->data[0] != (DATA) CANARY || this->data[this->capacity + 1] !=  (DATA) CANARY)
        return -2;
